...
2. **Data Sources**:
   ...
   - **Local Data Source** (`AuthLocalDataSource`): Manages persisting the Bearer access token locally on the device (e.g., via `SharedPreferences`).
3. **Repository Implementation** (`AuthRepositoryImpl`): Implements `AuthRepository`. It interacts with remote and local data sources to perform actions like signing in, storing the session token locally, signing out, and fetching the session user. It converts network models to domain entities and handles errors functionally by returning a `Left(Failure)` or `Right(Data)`.
